...
> project. At this checkpoint the project carries **no disclosed `sorry`**.  Rung 3's window
> node and exceptional finite tail were proved on 2026-09-08: the front-normalized theorem
> `threeBlock_length_eq_eight_of_acyclicParadoxical` now says that every acyclic paradoxical
> three-odd-block segment of the displayed block form has length `8`. Every Front-A closer
> remains unaffected:
...
| first-crossing route (`FrontA/FirstCrossing*.lean`) | `StoppingCorrect` (Terras's coefficient stopping-time conjecture, CST) and `CrossingExists` are explicit open `def`s; `conjecture_of_stoppingCorrect_and_crossingExists` wires them to Collatz and `noNontrivialCycle_of_stoppingCorrect` to Front B, trust base only |
| a CST failure is a near-cycle | **proved** 2026-09-19 (`FirstCrossingOneRun.lean`, `FirstCrossingRuns.lean`, trust base only): a start that survives its first crossing has `D·n + 2^m·(y−n) = numer` with `3(y−n) < a`, and with `r` odd runs `2^m·n^(r−1)·y ≤ 3^K·(n+1)^r`, so the overshoot is at most about `r` and `(2^m − 3^K)·n^r ≤ 3^K·((n+1)^r − n^r)` (Simons–de Weger's Lemma 4 in integer form) |
| CST on few-run crossings | **proved** 2026-09-19: one run unconditionally (`stoppingCorrect_oneRun`, Steiner's inequality from `sep_two_three`; also discharges `SteinerOneCircuit`); two runs (`descends_of_twoRun`), at most four runs (`descends_of_oddRunCount_le_four`) and at most fifty runs (`descends_of_oddRunCount_le_fifty`) under the explicit `def` hypothesis `CSTVerified` (Rozier–Terracol Cor. 5.4: CST verified to `2.8·10^19`); ledger = trust base + the Rhin-lite `native_decide` certificates.  The fifty-run rung certifies its `log₂3` bracket at denominators ~6·10^15 from 44 verified digits of `log 2`, `log 3` (`LogTwoThreeDigits.lean`), with no power-comparison certificate |
| a CST failure is a stopping-time outlier | **proved** 2026-09-19 (`FirstCrossingOutlier.lean`): a survivor of its first crossing with `K` odd steps has `3·rhinLiteSepC·n ≤ K^437`, so its stopping time exceeds `(3cn)^(1/437)` |
| ballot coalescence rigidity | **refuted** 2026-09-19 (`FrontA/BallotCoalescenceWitness.lean`, `native_decide`): `15231450875` and `15231450879` both have ballot 34-letter words with 22 odd steps and meet after 34 steps, so an integer can have two ballot ancestors of one shape; found by the Φ-walk probe `parity_reconstruction.py ballot-walk` |
| `ones_ge_of_survives` | **proved** 2026-09-19: under `CSTVerified`, any survivor has at least 492276 odd steps (`sep_strong_492276`), and at least 6234549927241963 when it has at most 96 odd runs (`ones_ge_of_survives_6e15`) |
...
| `threeBlock_length_eq_eight_of_acyclicParadoxical` | **proved** 2026-09-08: a kernel-checked finite certificate rejects the ten remaining ceiling-passing tuples at lengths `5,16,27` by their least realizing residues, completing the front-normalized rung-3 classification; the exceptional-tail theorem itself is trust-base clean |
