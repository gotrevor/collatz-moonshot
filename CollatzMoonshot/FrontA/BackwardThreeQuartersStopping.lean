...
/-!
# The recursive exponent-`3/4` stopping-line construction

This file discharges `ThreeQuartersRepeatOrStoppingGrowth`, the last pinned
`def` of the exponent-`3/4` renewal layer, and the final pinned corollary
`threeQuarters_stopping_card_bound`.  It is the direct structural port of
`BackwardTwoThirdsStopping.lean`: the construction expands a finite frontier of
`Fin 5 × ℕ` height states rooted at `(0, d)`, replacing every node with
endpoint at most `H` by its certified child set
(`exists_threeQuartersChildFinset`), and freezing every node whose endpoint
exceeds `H` as a first exit.
...
(`ThreeQuartersStoppingNode`, `ThreeQuartersStoppingFront`) and the three round
lemmas are re-derived here, over `InThreeQuartersHeightState`/`Fin 5` and the
exact real edge weight `((x)/(y))^(3/4)` telescoped by
`threeQuartersEdgeWeight_mul`.

The pinned target `threeQuarters_stopping_card_bound` and its constants
`1000000`, `19334101` are preserved verbatim.
-/
...
def ThreeQuartersStoppingNode (d H m : ℕ) (s : ThreeQuartersNode) : Prop :=
  InThreeQuartersHeightState d s.1 s.2 ∧ s.2 % 2 = 1 ∧ ¬3 ∣ s.2 ∧
    s.2 < 2 ^ 23 * H ∧
    ReachesInBand d (2 ^ 25 * H) s.2 ∧ StoppingChain d H m s.2

/-- The full frontier invariant at round `m`: value injectivity, per-node
state data, and strict telescoping exponent-`3/4` mass above the root
potential. -/
def ThreeQuartersStoppingFront (d H m : ℕ) (F : Finset ThreeQuartersNode) : Prop :=
  ThreeQuartersValuesInjective F ∧
    (∀ s ∈ F, ThreeQuartersStoppingNode d H m s) ∧
    (threeQuartersPotential 0 d : ℝ) <
      ∑ s ∈ F, threeQuartersEdgeWeight d s.2 * (threeQuartersPotential s.1 s.2 : ℝ)

/-- The root state `(0, d)` sits in its own height floor `1`. -/
theorem inThreeQuartersHeightState_root (d : ℕ) :
    InThreeQuartersHeightState d 0 d := by
  simp only [InThreeQuartersHeightState, threeQuartersHeightNum,
    threeQuartersHeightDen]
  norm_num

/-- The round-`1` frontier: the certified child set of the root itself. -/
theorem threeQuartersFront_init {d H : ℕ} (hd : 2 ≤ d) (hdodd : d % 2 = 1)
    (hd3 : ¬3 ∣ d) (hdH : d ≤ H) : ∃ F, ThreeQuartersStoppingFront d H 1 F := by
  obtain ⟨C, hCne, hCinj, hCchild, hCbound, hCmass⟩ :=
    exists_threeQuartersChildFinset (0 : Fin 5) hd hd hd3
      (inThreeQuartersHeightState_root d)
  have hd25 : d ≤ 2 ^ 25 * H :=
...
intact. -/
theorem threeQuartersFront_step {d H m : ℕ} (hd : 2 ≤ d) (_hdH : d ≤ H)
    {F : Finset ThreeQuartersNode} (hF : ThreeQuartersStoppingFront d H m F)
    (hL : (F.filter fun s => s.2 ≤ H).Nonempty) :
    (∃ n, d ≤ n ∧ OnCycle n) ∨ ∃ F', ThreeQuartersStoppingFront d H (m + 1) F' := by
  classical
  ...
  · -- no collision: replace every low node by its child set
    ...
    · -- strict mass growth
      ...
      have hexp := weighted_biUnion_expands L hLne Cf
        (fun s => threeQuartersEdgeWeight d s.2)
        (fun s => (threeQuartersPotential s.1 s.2 : ℝ))
        (fun s c => threeQuartersEdgeWeight s.2 c.2)
        hdisj
        (by
          intro s hs
        ...
        (by
          ...
          exact threeQuartersEdgeWeight_mul (by omega) (by omega))
      ...
      have hsplit :
          (∑ s ∈ F, threeQuartersEdgeWeight d s.2 *
              (threeQuartersPotential s.1 s.2 : ℝ)) =
            (∑ s ∈ L, threeQuartersEdgeWeight d s.2 *
              (threeQuartersPotential s.1 s.2 : ℝ)) +
            ∑ s ∈ E, threeQuartersEdgeWeight d s.2 *
              (threeQuartersPotential s.1 s.2 : ℝ) := by
        rw [hLdef, hEdef,
      ...
      calc
        ...
        _ = (∑ s ∈ L, threeQuartersEdgeWeight d s.2 *
              (threeQuartersPotential s.1 s.2 : ℝ)) +
            ∑ s ∈ E, threeQuartersEdgeWeight d s.2 *
              (threeQuartersPotential s.1 s.2 : ℝ) := hsplit
        _ < (∑ s ∈ L.biUnion Cf, threeQuartersEdgeWeight d s.2 *
              (threeQuartersPotential s.1 s.2 : ℝ)) +
            ∑ s ∈ E, threeQuartersEdgeWeight d s.2 *
              (threeQuartersPotential s.1 s.2 : ℝ) := by
              exact add_lt_add_of_lt_of_le hexp le_rfl
        _ = ∑ s ∈ E ∪ L.biUnion Cf, threeQuartersEdgeWeight d s.2 *
              (threeQuartersPotential s.1 s.2 : ℝ) := by
              rw [Finset.sum_union hEdisj]
              ring

/-- A frontier with no remaining low node is exactly the pinned exponent-`3/4`
stopping frontier. -/
theorem threeQuartersStoppingFrontier_of_no_low {d H m : ℕ}
    {F : Finset ThreeQuartersNode} (hF : ThreeQuartersStoppingFront d H m F)
    (hL : ¬(F.filter fun s => s.2 ≤ H).Nonempty) :
    ThreeQuartersStoppingFrontier d H F := by
  obtain ⟨hinj, hnode, hmass⟩ := hF
  refine ⟨hinj, hmass, ?_⟩
  intro s hs
...
/-- **The recursive exponent-`3/4` stopping-line theorem.**  Every odd reusable
root either exposes a positive Collatz cycle at or above its own floor, or
carries a finite first-exit frontier with the certified exponent-`3/4`
...
in `[d, H]`. -/
theorem threeQuartersRepeatOrStoppingGrowth : ThreeQuartersRepeatOrStoppingGrowth := by
  intro d H hd hdodd hd3 hdH
  obtain ⟨F₁, hF₁⟩ := threeQuartersFront_init hd hdodd hd3 hdH
  suffices h : ∀ k m (F : Finset ThreeQuartersNode),
      ThreeQuartersStoppingFront d H m F →
      H + 2 ≤ m + k →
      (∃ n, d ≤ n ∧ OnCycle n) ∨
        ∃ S : Finset ThreeQuartersNode, ThreeQuartersStoppingFrontier d H S by
  ...
  | zero =>
      ...
      · exfalso
        ...
        omega
      · exact Or.inr ⟨F, threeQuartersStoppingFrontier_of_no_low hF hL⟩
  | succ k ih =>
      intro m F hF hm
      by_cases hL : (F.filter fun s => s.2 ≤ H).Nonempty
      · rcases threeQuartersFront_step hd hdH hF hL with hcyc | ⟨F', hF'⟩
        · exact Or.inl hcyc
...
`threeQuartersRepeatOrStoppingGrowth` is impossible for an odd unit root
`d ≥ 2`. -/
theorem threeQuartersStoppingFrontier_of_noCycle (hnc : NoNontrivialCycle)
    {d H : ℕ} (hd : 2 ≤ d) (hdodd : d % 2 = 1) (hd3 : ¬3 ∣ d) (hdH : d ≤ H) :
    ∃ S : Finset ThreeQuartersNode, ThreeQuartersStoppingFrontier d H S := by
  rcases threeQuartersRepeatOrStoppingGrowth d H hd hdodd hd3 hdH with hcycle | hfront
  · obtain ⟨n, hdn, hncycle⟩ := hcycle
    have hn : 1 ≤ n := by omega
    rcases hnc n hn hncycle with rfl | rfl | rfl <;> omega
  · exact hfront

/-! ## A native-checked uniform lower bound on the potential -/

/-- Every unit residue of every floor has potential at least `1000000`. -/
theorem threeQuartersPotential_ge_min_fin :
    ∀ i : Fin 5, ∀ r : Fin 243, r.1 % 3 ≠ 0 →
      1000000 ≤ threeQuartersPotential i r.1 := by
  native_decide

/-- The frozen `3/4`-potential is uniformly at least `1000000` on unit
residues. -/
theorem threeQuartersPotential_ge_min {x : ℕ} (h3 : ¬3 ∣ x) (i : Fin 5) :
    1000000 ≤ threeQuartersPotential i x := by
  ...
  exact threeQuartersPotential_ge_min_fin i ⟨x % 243, hr243⟩ hru

/-! ## The explicit exponent-`3/4` stopping-frontier cardinal corollary -/
...
theorem threeQuarters_stopping_card_bound (hnc : NoNontrivialCycle)
  ...
  obtain ⟨S, hS⟩ :=
    threeQuartersStoppingFrontier_of_noCycle hnc hd hdodd hd3 hdH
  have hb := hS.card_bound (by omega) (by omega)
  have hmin : (1000000 : ℝ) ≤ (threeQuartersPotential 0 d : ℝ) := by
    exact_mod_cast threeQuartersPotential_ge_min hd3 0
