...
/-!
# The recursive exponent-`4/5` stopping-line construction

This file discharges `FourFifthsRepeatOrStoppingGrowth`, the last pinned
`def` of the exponent-`4/5` renewal layer, and the final pinned corollary
`fourFifths_stopping_card_bound`.  It is the direct structural port of
`BackwardThreeQuartersStopping.lean`: the construction expands a finite frontier of
`Fin 5 × ℕ` height states rooted at `(0, d)`, replacing every node with
endpoint at most `H` by its certified child set
(`exists_fourFifthsChildFinset`), and freezing every node whose endpoint
...
(`FourFifthsStoppingNode`, `FourFifthsStoppingFront`) and the three round
lemmas are re-derived here, over `InThreeQuartersHeightState`/`Fin 5` and the
exact real edge weight `((x)/(y))^(4/5)` telescoped by
`fourFifthsEdgeWeight_mul`.

The pinned target `fourFifths_stopping_card_bound` and its constants
`1000000`, `41080952` are preserved verbatim.
-/
...
def FourFifthsStoppingNode (d H m : ℕ) (s : FourFifthsNode) : Prop :=
...
state data, and strict telescoping exponent-`4/5` mass above the root
potential. -/
def FourFifthsStoppingFront (d H m : ℕ) (F : Finset FourFifthsNode) : Prop :=
  FourFifthsValuesInjective F ∧
    (∀ s ∈ F, FourFifthsStoppingNode d H m s) ∧
    (fourFifthsPotential 0 d : ℝ) <
      ∑ s ∈ F, fourFifthsEdgeWeight d s.2 * (fourFifthsPotential s.1 s.2 : ℝ)

/-- The root state `(0, d)` sits in its own height floor `1`. -/
theorem inFourFifthsHeightState_root (d : ℕ) :
...
theorem fourFifthsFront_init {d H : ℕ} (hd : 2 ≤ d) (hdodd : d % 2 = 1)
    (hd3 : ¬3 ∣ d) (hdH : d ≤ H) : ∃ F, FourFifthsStoppingFront d H 1 F := by
  obtain ⟨C, hCne, hCinj, hCchild, hCbound, hCmass⟩ :=
    exists_fourFifthsChildFinset (0 : Fin 5) hd hd hd3
      (inFourFifthsHeightState_root d)
  have hd25 : d ≤ 2 ^ 25 * H :=
...
intact. -/
theorem fourFifthsFront_step {d H m : ℕ} (hd : 2 ≤ d) (_hdH : d ≤ H)
    {F : Finset FourFifthsNode} (hF : FourFifthsStoppingFront d H m F)
    (hL : (F.filter fun s => s.2 ≤ H).Nonempty) :
    (∃ n, d ≤ n ∧ OnCycle n) ∨ ∃ F', FourFifthsStoppingFront d H (m + 1) F' := by
  classical
  ...
  · -- no collision: replace every low node by its child set
    ...
    · -- strict mass growth
      ...
      have hexp := weighted_biUnion_expands L hLne Cf
        (fun s => fourFifthsEdgeWeight d s.2)
        (fun s => (fourFifthsPotential s.1 s.2 : ℝ))
        (fun s c => fourFifthsEdgeWeight s.2 c.2)
        hdisj
        (by
          intro s hs
        ...
        (by
          ...
          exact fourFifthsEdgeWeight_mul (by omega) (by omega))
      ...
      have hsplit :
          (∑ s ∈ F, fourFifthsEdgeWeight d s.2 *
              (fourFifthsPotential s.1 s.2 : ℝ)) =
            (∑ s ∈ L, fourFifthsEdgeWeight d s.2 *
              (fourFifthsPotential s.1 s.2 : ℝ)) +
            ∑ s ∈ E, fourFifthsEdgeWeight d s.2 *
              (fourFifthsPotential s.1 s.2 : ℝ) := by
        rw [hLdef, hEdef,
      ...
      calc
        ...
        _ = (∑ s ∈ L, fourFifthsEdgeWeight d s.2 *
              (fourFifthsPotential s.1 s.2 : ℝ)) +
            ∑ s ∈ E, fourFifthsEdgeWeight d s.2 *
              (fourFifthsPotential s.1 s.2 : ℝ) := hsplit
        _ < (∑ s ∈ L.biUnion Cf, fourFifthsEdgeWeight d s.2 *
              (fourFifthsPotential s.1 s.2 : ℝ)) +
            ∑ s ∈ E, fourFifthsEdgeWeight d s.2 *
              (fourFifthsPotential s.1 s.2 : ℝ) := by
              exact add_lt_add_of_lt_of_le hexp le_rfl
        _ = ∑ s ∈ E ∪ L.biUnion Cf, fourFifthsEdgeWeight d s.2 *
              (fourFifthsPotential s.1 s.2 : ℝ) := by
              rw [Finset.sum_union hEdisj]
              ring

/-- A frontier with no remaining low node is exactly the pinned exponent-`4/5`
stopping frontier. -/
theorem fourFifthsStoppingFrontier_of_no_low {d H m : ℕ}
    {F : Finset FourFifthsNode} (hF : FourFifthsStoppingFront d H m F)
    (hL : ¬(F.filter fun s => s.2 ≤ H).Nonempty) :
    FourFifthsStoppingFrontier d H F := by
  obtain ⟨hinj, hnode, hmass⟩ := hF
  refine ⟨hinj, hmass, ?_⟩
  intro s hs
...
/-- **The recursive exponent-`4/5` stopping-line theorem.**  Every odd reusable
root either exposes a positive Collatz cycle at or above its own floor, or
carries a finite first-exit frontier with the certified exponent-`4/5`
...
in `[d, H]`. -/
theorem fourFifthsRepeatOrStoppingGrowth : FourFifthsRepeatOrStoppingGrowth := by
  intro d H hd hdodd hd3 hdH
  obtain ⟨F₁, hF₁⟩ := fourFifthsFront_init hd hdodd hd3 hdH
  suffices h : ∀ k m (F : Finset FourFifthsNode),
      FourFifthsStoppingFront d H m F →
      H + 2 ≤ m + k →
      (∃ n, d ≤ n ∧ OnCycle n) ∨
        ∃ S : Finset FourFifthsNode, FourFifthsStoppingFrontier d H S by
  ...
  | zero =>
      ...
      · exfalso
        ...
        omega
      · exact Or.inr ⟨F, fourFifthsStoppingFrontier_of_no_low hF hL⟩
  | succ k ih =>
      intro m F hF hm
      by_cases hL : (F.filter fun s => s.2 ≤ H).Nonempty
      · rcases fourFifthsFront_step hd hdH hF hL with hcyc | ⟨F', hF'⟩
        · exact Or.inl hcyc
...
`fourFifthsRepeatOrStoppingGrowth` is impossible for an odd unit root
`d ≥ 2`. -/
theorem fourFifthsStoppingFrontier_of_noCycle (hnc : NoNontrivialCycle)
    {d H : ℕ} (hd : 2 ≤ d) (hdodd : d % 2 = 1) (hd3 : ¬3 ∣ d) (hdH : d ≤ H) :
    ∃ S : Finset FourFifthsNode, FourFifthsStoppingFrontier d H S := by
  rcases fourFifthsRepeatOrStoppingGrowth d H hd hdodd hd3 hdH with hcycle | hfront
...
theorem fourFifthsPotential_ge_min_fin :
    ∀ i : Fin 5, ∀ r : Fin 6561, r.1 % 3 ≠ 0 →
      1000000 ≤ fourFifthsPotential i r.1 := by
  native_decide

/-- The frozen `4/5`-potential is uniformly at least `1000000` on unit
residues. -/
theorem fourFifthsPotential_ge_min {x : ℕ} (h3 : ¬3 ∣ x) (i : Fin 5) :
    1000000 ≤ fourFifthsPotential i x := by
  ...
  exact fourFifthsPotential_ge_min_fin i ⟨x % 6561, hr6561⟩ hru

/-! ## The explicit exponent-`4/5` stopping-frontier cardinal corollary -/
...
theorem fourFifths_stopping_card_bound (hnc : NoNontrivialCycle)
  ...
  obtain ⟨S, hS⟩ :=
    fourFifthsStoppingFrontier_of_noCycle hnc hd hdodd hd3 hdH
  have hb := hS.card_bound (by omega) (by omega)
  have hmin : (1000000 : ℝ) ≤ (fourFifthsPotential 0 d : ℝ) := by
    exact_mod_cast fourFifthsPotential_ge_min hd3 0
